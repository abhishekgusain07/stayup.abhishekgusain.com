...
export function useAlertRecipients(monitorId: string) {
  ...
  const fetchRecipients = useCallback(async () => {
    ...
    try {
      ...
      const errorMessage =
        err instanceof Error ? err.message : "Failed to fetch alert recipients";
  ...
  const addRecipient = useCallback(
    async (email: string) => {
      try {
        const response = await apiClient.addAlertRecipient(monitorId, {
          email,
        });
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Failed to add alert recipient";
        toast.error(errorMessage);
        throw err;
      }
    },
  ...
  const removeRecipient = useCallback(
    async (recipientId: string) => {
      try {
        const response = await apiClient.removeAlertRecipient(
          monitorId,
          recipientId
        );
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Failed to remove alert recipient";
        toast.error(errorMessage);
        throw err;
      }
    },
  ...
  const toggleRecipient = useCallback(
    async (recipientId: string) => {
      try {
        const response = await apiClient.toggleAlertRecipient(
          monitorId,
          recipientId
        );
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Failed to toggle alert recipient";
        toast.error(errorMessage);
        throw err;
      }
    },

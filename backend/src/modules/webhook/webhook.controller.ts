...
export class WebhookController {
  ...
  @ApiOperation({
    ...
    description:
      "Internal endpoint for Lambda functions to post monitoring results back to the main API",
  })
  @ApiHeader({
    name: "x-api-secret",
  ...
  async receiveMonitorResults(
    ...
    try {
      ...
      await this.webhookService.processMonitorResults(
        webhookPayload,
        apiSecret,
      );
    ...
    try {
      ...
      if (health.status === "healthy") {
        ...
        throw new HttpException(
          createErrorResponse(
            "Service unhealthy",
            health.error || "Webhook service is not functioning properly",
          ),
  ...
  @ApiOperation({
    ...
    description:
      "Test endpoint for validating webhook functionality during development",
  })
  async testWebhook(

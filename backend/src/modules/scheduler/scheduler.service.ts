...
export class SchedulerService {
  ...
  private initializeSQSClients(): void {
    ...
    console.log(
      "SQS clients initialized for regions:",
      Array.from(this.sqsClients.keys()),
    );
  }
  ...
  async scheduleMonitoringJobs(): Promise<void> {
    try {
      ...
      console.log(
        `📋 Found ${monitorsToCheck.length} monitors due for checking`,
      );

      // Distribute jobs across regions
      const jobsByRegion =
        await this.distributeJobsAcrossRegions(monitorsToCheck);
      ...
      console.log(
        `✅ Scheduler completed: ${totalJobsSent} jobs sent across ${Object.keys(jobsByRegion).length} regions in ${duration}ms`,
      );
    } catch (error) {
   ...
   */
  private async distributeJobsAcrossRegions(
    monitors: any[],
  ): Promise<Record<string, MonitorJob[]>> {
    ...
    const totalJobs = Object.values(jobsByRegion).reduce(
      (sum, jobs) => sum + jobs.length,
      0,
    );
    console.log(
      `📦 Created ${totalJobs} monitoring jobs across ${regions.length} regions`,
    );
   ...
   */
  private async sendJobsToSQS(
    jobsByRegion: Record<string, MonitorJob[]>,
  ): Promise<number> {
    let totalJobsSent = 0;
   ...
   */
  private async sendJobsToRegionQueue(
    region: string,
    jobs: MonitorJob[],
  ): Promise<number> {
    try {
      ...
      if (!sqsClient || !queueUrl) {
        console.error(
          `SQS client or queue URL not found for region: ${region}`,
        );
        return 0;
      }
      ...
      for (let i = 0; i < jobs.length; i += batchSize) {
        ...
        console.log(
          `📤 Sent ${batch.length} jobs to ${region} (total: ${jobsSent}/${jobs.length})`,
        );
      }
  ...
  async getSchedulerStats(): Promise<any> {
    try {
      const activeMonitors = await this.db
        .select({ count: count() })
        .from(monitors)
        .where(and(eq(monitors.isActive, true)));
